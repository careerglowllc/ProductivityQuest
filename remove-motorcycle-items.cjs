/**
 * Remove "Motorcycle Registration" and "Motorcycle Insurance (Progressive)"
 * financial items for alexbaer321@gmail.com (motorcycle was sold).
 *
 * Run:  node remove-motorcycle-items.cjs
 */
const fs = require('fs');
  ...
  try {
    ...
    const match = await pool.query(
      `SELECT id, item, monthly_cost FROM financial_items
       WHERE user_id = $1 AND (item ILIKE '%Motorcycle Registration%' OR item ILIKE '%Motorcycle Insurance%')`,
      [userId]
    );

    if (match.rows.length === 0) {
      console.log('ℹ️  No matching "Motorcycle Registration" / "Motorcycle Insurance" items found (already removed?).');
      return;
    }
