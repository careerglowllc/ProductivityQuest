// Seed finance data — "Routine Income Expense May 2026 Backup.csv"
// Usage: node -r dotenv/config seed-finances.cjs
// Looks up alexbaer321@gmail.com automatically.
...
const FINANCE_DATA = [
  ...
  { item: "Phone Bill (data and call, my part of dad's family plan)", category: "Phone", monthlyCost: 3600, recurType: "Monthly" },
  { item: "Toiletries/Hygiene", category: "Toiletries", monthlyCost: 1500, recurType: "Monthly" },
  { item: "Electricity and Gas Bill (PG&E Solar/NEM)", category: "Housing", monthlyCost: 30000, recurType: "Monthly" },
  ...
  { item: "Chase Sapphire Preferred Annual Fee", category: "Credit Card", monthlyCost: 792, recurType: "Yearly (Amortized)" },
  ...
  { item: "Replit Core (CareerGlow)", category: "Business", monthlyCost: 2000, recurType: "Monthly" },
  { item: "401k Apple's Matching Contribution", category: "Retirement", monthlyCost: 73034, recurType: "Biweekly (Summed Monthly)" },
  { item: "Apple RSUs (after-tax rough amortization)", category: "Income", monthlyCost: 430000, recurType: "Monthly" },
  ...
  { item: "reviewer@mycareerglow.com Google Workspace", category: "Business", monthlyCost: 700, recurType: "Monthly" },
...
async function seedFinances() {
  ...
  try {
    ...
    const userRows = await sql`SELECT id FROM users WHERE email = 'alexbaer321@gmail.com' LIMIT 1`;
    if (userRows.length === 0) {
      console.error('❌ User alexbaer321@gmail.com not found in database.');
      return;
    }
    const userId = userRows[0].id;
    console.log(`\n💰 Seeding finance data for user: ${userId} (alexbaer321@gmail.com)\n`);
