...
type AtlasNode = { id: string; title: string; parentId: string | null; order: number; completed: boolean; createdAt: string };
type AtlasProject = { id: string; title: string; description: string; nodes: AtlasNode[] };
type Editor = { id: string | null; parentId: string | null; order?: number };
...
const starter: AtlasProject = { id: "atlas-starter", title: "Build a life with more room in it", description: "A private observatory for the work that makes the rest of life possible.", nodes: [
  { id: "health", title: "Restore the body", parentId: null, order: 0, completed: false, createdAt: "2025-01-01" },
  { id: "health-1", title: "Find a sustainable morning", parentId: "health", order: 0, completed: true, createdAt: "2025-01-01" },
  { id: "health-2", title: "Make movement ordinary", parentId: "health", order: 1, completed: false, createdAt: "2025-01-01" },
  { id: "craft", title: "Make useful things", parentId: null, order: 1, completed: false, createdAt: "2025-01-01" },
  { id: "craft-1", title: "Choose the next small tool", parentId: "craft", order: 0, completed: false, createdAt: "2025-01-01" },
] };
...
function repair(raw: unknown): AtlasProject {
  ...
  source.forEach((item) => {
    ...
    nodes.push({ id: node.id, title: typeof node.title === "string" && node.title.trim() ? node.title.trim() : "Untitled task", parentId: typeof node.parentId === "string" ? node.parentId : null, order: Number.isInteger(node.order) && node.order >= 0 ? node.order : Number.NaN, completed: node.completed === true, createdAt: typeof node.createdAt === "string" ? node.createdAt : new Date().toISOString() });
  });
...
function AtlasEditor({ open, onOpenChange, editor, project, draft, setDraft, description, setDescription, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; editor: Editor | null; project: AtlasProject; draft: string; setDraft: (value: string) => void; description: string; setDescription: (value: string) => void; onSave: () => void }) {
  if (!editor) return null;
  const editingRoot = editor.id === "root";
   return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="atlas-dialog"><DialogHeader><DialogTitle>{editor.id ? "Edit task" : editor.parentId ? "Add sub-task" : "Add task"}</DialogTitle><DialogDescription>{editor.id ? "Refine the title. Its position in the tree keeps the parent relationship intact." : editor.parentId ? "This sub-task will be registered directly beneath its parent." : "This task will branch directly from the north star."}</DialogDescription></DialogHeader>
    ...
    <DialogFooter><button type="button" className="atlas-secondary-button" onClick={() => onOpenChange(false)}>Cancel</button><button type="button" className="atlas-primary-button" disabled={!draft.trim()} onClick={onSave}>{editor.id ? "Save changes" : editor.parentId ? "Add sub-task" : "Add task"}</button></DialogFooter>
...
export default function QuestlineAtlas() {
  ...
  const beginBranch = (parentId: string | null, order: number) => {
    beginEdit({ id: null, parentId, order }, "");
  };
  const save = () => {
    ...
    setProject((current) => editor.id ? { ...current, nodes: current.nodes.map((node) => node.id === editor.id ? { ...node, title: draft.trim(), parentId: editor.parentId } : node) } : { ...current, nodes: [...current.nodes, { id: newId!, title: draft.trim(), parentId: editor.parentId, order: editor.order ?? current.nodes.filter((node) => node.parentId === editor.parentId).length, completed: false, createdAt: new Date().toISOString() }] });
    if (newId) setSelected(newId);
    setEditor(null); setNotice(editor.id ? "Task updated." : editor.parentId ? "Sub-task added." : "Task added.");
  };
  const remove = (nodeId: string) => {
    if (!window.confirm("Delete this task and every sub-task beneath it?")) return;
  ...
  return <main className="atlas-page"><header className="atlas-header"><div><p className="atlas-kicker"><GitBranch size={14} /> Private planning observatory</p><h1>Questline Atlas</h1><p className="atlas-intro">One north star. The tasks and nested sub-tasks that make it reachable.</p></div><div className="atlas-header-actions"><button className="atlas-quiet-button" onClick={() => reset(false)}><RotateCcw size={15} /> Reset example</button><button className="atlas-danger-button" onClick={() => reset(true)}><Trash2 size={15} /> Clear atlas</button></div></header>
     <section className="atlas-layout"><div className="atlas-scene-frame">{layout.width > width + 8 && <div className="atlas-scroll-cue">Swipe sideways to explore all branches</div>}<div ref={sceneViewportRef} className={`atlas-scene-scroll ${layout.width > width + 8 ? "is-scrollable" : ""}`}><div className="atlas-scene" style={{ width: layout.width, height: layout.height }} aria-label="Questline task tree"><div className="atlas-stars" /><svg className="atlas-connections" width={layout.width} height={layout.height} aria-hidden="true">{Array.from(layout.positions.entries()).filter(([nodeId]) => nodeId !== "root").map(([nodeId, point]) => { const node = project.nodes.find((item) => item.id === nodeId)!; const parent = layout.positions.get(node.parentId || "root")!; return <path key={nodeId} className={node.completed ? "is-complete" : ""} d={`M ${parent.x} ${parent.y} C ${parent.x} ${parent.y + 62}, ${point.x} ${point.y - 62}, ${point.x} ${point.y}`} />; })}{layout.addPoints.map((point) => { const parent = layout.positions.get(point.parentId || "root")!; return <path key={`add-${point.parentId || "root"}-${point.order}`} className="is-open-branch" d={`M ${parent.x} ${parent.y} C ${parent.x} ${parent.y + 62}, ${point.x} ${point.y - 54}, ${point.x} ${point.y}`} />; })}</svg>
      <div className="atlas-root" style={{ left: layout.positions.get("root")!.x, top: layout.positions.get("root")!.y }}><button className={`atlas-orb atlas-root-orb ${selected === "root" ? "is-selected" : ""}`} onClick={() => setSelected("root")} aria-label={`Select ${project.title}`}><GitBranch size={23} /></button><strong>{project.title}</strong><span>{completed} / {project.nodes.length} illuminated</span></div>
       {project.nodes.map((node) => { const point = layout.positions.get(node.id)!; return <button key={node.id} className={`atlas-orbit-node ${node.completed ? "is-complete" : ""} ${selected === node.id ? "is-selected" : ""}`} style={{ left: point.x, top: point.y } as CSSProperties} onClick={() => setSelected(node.id)} aria-label={`${node.title}, ${node.parentId ? "sub-task" : "task"}, ${node.completed ? "complete" : "in progress"}`}><span className="atlas-orb"><i>{node.completed ? <Check size={12} /> : <Circle size={8} />}</i></span><b>{node.title}</b><small>{node.parentId ? "sub-task" : "task"}</small></button>; })}
       {layout.addPoints.map((point) => <button key={`branch-${point.parentId || "root"}-${point.order}`} type="button" className="atlas-add-branch" style={{ left: point.x, top: point.y } as CSSProperties} onClick={() => beginBranch(point.parentId, point.order)} aria-label={`Add ${point.parentId ? "sub-task" : "task"} to ${point.parentId ? project.nodes.find((node) => node.id === point.parentId)?.title : project.title}`} title={point.parentId ? "Add sub-task" : "Add task"}><Plus size={15} aria-hidden="true" /></button>)}
       {!project.nodes.length && <div className="atlas-empty"><p>Start your first branch</p><span>Use either + below the north star. Each new task belongs to the task directly above it.</span></div>}<div className="atlas-scene-footer"><span>{project.nodes.length} {project.nodes.length === 1 ? "task" : "tasks"} · Use + to build the next branch</span></div></div></div></div>
        <aside className="atlas-inspector">{selectedNode ? <div><span className="atlas-panel-label">{selectedNode.parentId ? "Sub-task" : "Task"} selected</span><h2>{selectedNode.title}</h2><p className="atlas-parent">Child of {project.nodes.find((node) => node.id === selectedNode.parentId)?.title || project.title}</p><div className="atlas-selection-guide"><strong>Tree position defines the relationship</strong><span>The + beneath this task creates a sub-task. Adjacent tasks are siblings, and every sub-task can branch again.</span></div><div className={`atlas-status ${selectedNode.completed ? "is-complete" : ""}`}>{selectedNode.completed ? <Check size={14} /> : <Circle size={12} />}{selectedNode.completed ? "Complete" : "In progress"}</div><div className="atlas-action-stack"><button className="atlas-secondary-button" onClick={() => setProject((current) => ({ ...current, nodes: current.nodes.map((node) => node.id === selectedNode.id ? { ...node, completed: !node.completed } : node) }))}>{selectedNode.completed ? "Mark in progress" : "Mark complete"}</button><button className="atlas-secondary-button" onClick={() => beginEdit({ id: selectedNode.id, parentId: selectedNode.parentId, order: selectedNode.order }, selectedNode.title)}><Pencil size={14} /> Edit task</button><button className="atlas-delete-link" onClick={() => remove(selectedNode.id)}><Trash2 size={14} /> Delete branch</button></div></div> : <div className="atlas-project-panel"><span className="atlas-panel-label">North star</span><h2>{project.title}</h2><p>{project.description}</p><div className="atlas-selection-guide"><strong>Build directly in the tree</strong><span>Add tasks beneath the north star, then keep nesting sub-tasks for as many levels as needed.</span></div><button className="atlas-secondary-button atlas-edit-root" onClick={() => beginEdit({ id: "root", parentId: null }, project.title)}><Pencil size={15} /> Edit north star</button></div>}{notice && <p className="atlas-notice" role="status">{notice}</p>}<div className="atlas-legend"><span><i className="legend-task" /> Task or sub-task</span><span><i className="legend-done"><Check size={10} /></i> Complete</span></div></aside></section>
    <AtlasEditor open={!!editor} onOpenChange={(open) => !open && setEditor(null)} editor={editor} project={project} draft={draft} setDraft={setDraft} description={draftDescription} setDescription={setDraftDescription} onSave={() => { if (editor?.id === "root") { setProject((current) => ({ ...current, title: draft.trim() || current.title, description: draftDescription.trim() })); setEditor(null); setNotice("Root questline updated."); } else save(); }} />
  </main>;
}