...
function AtlasEditor({ open, onOpenChange, editor, setEditor, project, draft, setDraft, description, setDescription, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; editor: Editor | null; setEditor: (editor: Editor) => void; project: AtlasProject; draft: string; setDraft: (value: string) => void; description: string; setDescription: (value: string) => void; onSave: () => void }) {
  ...
  const choices = project.nodes.filter((node) => node.id !== editor.id && !isDescendant(project.nodes, node.id, editor.id || ""));
   return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="atlas-dialog"><DialogHeader><DialogTitle>{editor.id ? "Shape the next branch" : editor.parentId ? "Add a child point" : "Add a root point"}</DialogTitle><DialogDescription>{editor.id ? "Rename this point or move it to another orbit." : editor.parentId ? "This point will be nested beneath the selected point." : "This point will branch directly from the north star."}</DialogDescription></DialogHeader>
...
export default function QuestlineAtlas() {
  ...
  const layout = useMemo(() => {
    ...
    // Let quiet and small atlases breathe at viewport width; only dense levels
    // earn a wider field that users can intentionally pan across.
    const responsiveMinimum = width;
  ...
  const save = () => {
    ...
    setProject((current) => editor.id ? { ...current, nodes: current.nodes.map((node) => node.id === editor.id ? { ...node, title: draft.trim(), parentId: editor.parentId } : node) } : { ...current, nodes: [...current.nodes, { id: newId!, title: draft.trim(), parentId: editor.parentId, kind: editor.kind, completed: false, createdAt: new Date().toISOString() }] });
  ...
  return <main className="atlas-page"><header className="atlas-header"><div><p className="atlas-kicker"><GitBranch size={14} /> Private planning observatory</p><h1>Questline Atlas</h1><p className="atlas-intro">One north star. The stages and quests that make it reachable.</p></div><div className="atlas-header-actions"><button className="atlas-quiet-button" onClick={() => reset(false)}><RotateCcw size={15} /> Reset example</button><button className="atlas-danger-button" onClick={() => reset(true)}><Trash2 size={15} /> Clear atlas</button></div></header>
     <section className="atlas-layout"><div className="atlas-scene-frame">{layout.width > width + 8 && <div className="atlas-scroll-cue">Swipe sideways to explore all branches</div>}<div ref={sceneViewportRef} className={`atlas-scene-scroll ${layout.width > width + 8 ? "is-scrollable" : ""}`}><div className="atlas-scene" style={{ width: layout.width, height: layout.height }} aria-label="Questline constellation tree"><div className="atlas-stars" /><svg className="atlas-connections" width={layout.width} height={layout.height} aria-hidden="true">{Array.from(layout.positions.entries()).filter(([nodeId]) => nodeId !== "root").map(([nodeId, point]) => { const node = project.nodes.find((item) => item.id === nodeId)!; const parent = layout.positions.get(node.parentId || "root")!; return <path key={nodeId} className={node.completed ? "is-complete" : ""} d={`M ${parent.x} ${parent.y} C ${parent.x} ${parent.y + 62}, ${point.x} ${point.y - 62}, ${point.x} ${point.y}`} />; })}</svg>
      ...
      {project.nodes.map((node) => { const point = layout.positions.get(node.id)!; return <button key={node.id} className={`atlas-orbit-node ${node.kind} ${node.completed ? "is-complete" : ""} ${selected === node.id ? "is-selected" : ""}`} style={{ left: point.x, top: point.y } as CSSProperties} onClick={() => setSelected(node.id)} aria-label={`${node.title}, ${node.completed ? "complete" : "in progress"}`}><span className="atlas-orb"><i>{node.completed ? <Check size={12} /> : <Circle size={8} />}</i></span><b>{node.title}</b><small>{node.kind}</small></button>; })}
       {!project.nodes.length && <div className="atlas-empty"><p>The field is quiet.</p><span>Start with a root stage, then select any point to branch beneath it.</span><button onClick={() => beginEdit({ id: null, parentId: null, kind: "stage" }, "")}><Plus size={15} /> Add root stage</button></div>}<div className="atlas-scene-footer"><span>Atlas field · {project.nodes.length + 1} {project.nodes.length ? "points" : "point"}</span><button onClick={() => beginEdit({ id: null, parentId: null, kind: "stage" }, "")}><Plus size={14} /> Root stage</button><button onClick={() => beginEdit({ id: null, parentId: null, kind: "quest" }, "")}><Plus size={14} /> Root quest</button></div></div></div></div>
       <aside className="atlas-inspector">{selectedNode ? <div><span className="atlas-panel-label">{selectedNode.kind} point selected</span><h2>{selectedNode.title}</h2><p className="atlas-parent">Orbiting {project.nodes.find((node) => node.id === selectedNode.parentId)?.title || "the root questline"}</p><div className="atlas-selection-guide"><strong>Selected point</strong><span>New points will orbit beneath it.</span></div><div className={`atlas-status ${selectedNode.completed ? "is-complete" : ""}`}>{selectedNode.completed ? <Check size={14} /> : <Circle size={12} />}{selectedNode.completed ? "Complete" : "In progress"}</div><div className="atlas-action-stack"><button className="atlas-primary-button" onClick={() => beginEdit({ id: null, parentId: selectedNode.id, kind: "quest" }, "")}><Plus size={15} /> Add child quest</button><button className="atlas-secondary-button" onClick={() => beginEdit({ id: null, parentId: selectedNode.id, kind: "stage" }, "")}><Plus size={14} /> Add child stage</button><button className="atlas-secondary-button" onClick={() => setProject((current) => ({ ...current, nodes: current.nodes.map((node) => node.id === selectedNode.id ? { ...node, completed: !node.completed } : node) }))}>{selectedNode.completed ? "Mark in progress" : "Mark complete"}</button><button className="atlas-secondary-button" onClick={() => beginEdit({ id: selectedNode.id, parentId: selectedNode.parentId, kind: selectedNode.kind }, selectedNode.title)}><Pencil size={14} /> Rename or reparent</button><button className="atlas-delete-link" onClick={() => remove(selectedNode.id)}><Trash2 size={14} /> Delete branch</button></div></div> : <div className="atlas-project-panel"><span className="atlas-panel-label">North star</span><h2>{project.title}</h2><p>{project.description}</p><div className="atlas-selection-guide"><strong>Choose a starting point</strong><span>Root items sit beside the north star. Select a node to add children beneath it.</span></div><div className="atlas-root-actions"><button className="atlas-primary-button" onClick={() => beginEdit({ id: null, parentId: null, kind: "stage" }, "")}><Plus size={15} /> Add root stage</button><button className="atlas-secondary-button" onClick={() => beginEdit({ id: null, parentId: null, kind: "quest" }, "")}><Plus size={14} /> Add root quest</button></div><button className="atlas-secondary-button atlas-edit-root" onClick={() => beginEdit({ id: "root", parentId: null, kind: "stage" }, project.title)}><Pencil size={15} /> Edit root questline</button></div>}{selected === "root" && <div className="atlas-root-detail"><span className="atlas-panel-label">North star selected</span><p>Root items are added beside this north star. Select a stage or quest to create a child beneath it.</p><button className="atlas-primary-button" onClick={() => beginEdit({ id: null, parentId: null, kind: "stage" }, "")}><Plus size={15} /> Add root stage</button><button className="atlas-secondary-button" onClick={() => beginEdit({ id: null, parentId: null, kind: "quest" }, "")}><Plus size={14} /> Add root quest</button></div>}{notice && <p className="atlas-notice" role="status">{notice}</p>}<div className="atlas-legend"><span><i className="legend-stage" /> Stage</span><span><i className="legend-quest" /> Quest</span><span><i className="legend-done"><Check size={10} /></i> Complete</span></div></aside></section>
