  ...
  const layout = useMemo(() => {
    ...
    const place = (nodeId: string, parentId: string | null, level: number) => {
      ...
      const branchOrders = children.length === 0
        ? [0, 1]
        : children.length === 1
          ? [children[0].order, children[0].order === 0 ? 1 : 0].sort((a, b) => a - b)
          : children.map((child) => child.order);
