/**
 * Rename "GitHub Copilot Pro" business expense to "GitHub Copilot Pro +" and
 * update its cost to $40/mo for alexbaer321@gmail.com.
 *
 * Run:  node update-github-copilot-pro-plus.cjs
 */
const fs = require('fs');
...
const NEW_MONTHLY_COST_CENTS = 4000; // $40.00/mo
  ...
  try {
    ...
    const match = await pool.query(
      `SELECT id, item, monthly_cost FROM financial_items
       WHERE user_id = $1 AND item ILIKE '%GitHub Copilot Pro%'`,
      [userId]
    );

    if (match.rows.length === 0) {
      console.log('ℹ️  No matching "GitHub Copilot Pro" item found.');
      return;
    }
